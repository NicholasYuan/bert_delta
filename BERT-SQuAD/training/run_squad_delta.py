...
from transformers import (WEIGHTS_NAME, BertConfig,
                                  ...
                                  BertForQuestionAnswering_delta,
...
MODEL_CLASSES = {
    'bertdelta': (BertConfig, BertForQuestionAnswering_delta, BertTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in model.named_parameters() 
                    if (not any(nd in n for nd in no_decay) and (delta_params not in n ) )
                ], 
            'weight_decay': args.weight_decay
        },
        {'params': [p for n, p in model.named_parameters() 
                    if (any(nd in n for nd in no_decay) and (delta_params not in n ) )
                ], 
            'weight_decay': 0.0}
        ]
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if reg_type == 'none' :
                ...
                if global_step % args.print_step == 0:
                    logger.info('train','epoch:',_epoch,'\tloss:',loss,'' )
            else:
                delta_params_fill_0(model)
                ...
                for delta_step in trange(args.delta_steps, desc="delta_update"):
                    ...
                    tb_writer.add_scalar('perturbed loss', (loss), global_step)
                ...
                if global_step % args.print_step == 0:
                    logger.info('perturb','epoch:',_epoch,'\tloss:',loss,'')
...
def main():
    ...
    # all-layer margin 
    parser.add_argument('--all_layer_margin', action='store_true',
                        help="use all_layer_margin")
    # optimization params
    parser.add_argument("--weight_decay_delta", default=0.0, type=float,
                        help="Weight deay if we apply some for ALM.")
    parser.add_argument("--learning_rate_delta", default=5e-5, type=float,
                        help="The initial learning rate for Adam, ALM.")
    parser.add_argument("--adam_epsilon_delta", default=1e-8, type=float,
                        help="Epsilon for Adam optimizer, ALM.")
    parser.add_argument("--warmup_steps_delta", default=0, type=int,
                        help="Linear warmup over warmup_steps.")
    parser.add_argument('--switch_time', type=int, default=1 ,
    ...
    # set all layer margin
    config.set_delta(args.all_layer_margin)
